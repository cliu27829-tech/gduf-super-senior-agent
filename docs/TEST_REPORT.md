...
Playwright 自动化使用本机 Edge 和真实 5173/8000 服务完整跑通：注册 → 刷新登录 → 模型状态 → 普通对话 → 学习建议 → 多轮记忆 → 饭堂工具与来源 → 通知任务 → ICS → 重新登录 → 清理账号。自动化中的 FastAPI 模型请求运行在 8001 的 OpenAI 协议 Mock 服务，始终不读取真实 Key。项目所有者另行配置本机新 Key 后完成了真实供应商四轮浏览器冒烟：4/4 返回 200、页面与 API 回答一致、8 条历史消息持久化、多轮记忆通过；人工输入、响应全文与 Key 均不提交到仓库。
